    ...
    public class FanficController : Controller
    {
        ...
        public IActionResult ExportToPdf(string returnUrl, int id)
        {
            ...
            try
            {
                ...
                BaseFont baseFont = BaseFont.CreateFont(@"/font/arial.ttf", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
                string path = @"/pdf/tester1.pdf";
                file = new FileStream(path, FileMode.Create);
                wri = PdfWriter.GetInstance(pdfDoc, file);
                pdfDoc.Open();
                var spacer = new Paragraph("\n");
                FileStream fileLogo = new FileStream(@"/images/icons/logoPDF.png", FileMode.Open);
                ...
                Byte[] FileBuffer = User.DownloadData(@"/pdf/tester1.pdf");
                if (FileBuffer != null)
                {
            ...
            {                
                System.IO.File.Delete(@"/pdf/tester1.pdf");
            }            
